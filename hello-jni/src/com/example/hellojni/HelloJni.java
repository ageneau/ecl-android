...
public class HelloJni extends Activity
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
		...
		try
		{
            ...
            if(!uncompressedFilesDir.exists())
            {
                uncompressedFilesDir.mkdir();
            }

            if(DEBUG)
            {
                Log.w(TAG,"Uncompressing: " + "init.lsp");
                uncompressFile("init.lsp");
            }
            else
            {
                String[] files = assetManager.list(RESOURCES_DIR);
                Log.w(TAG,"Uncompressing: " + files.length + " files");
                for(int i=0; i<files.length; i++)
                {
                    Log.w(TAG,"Uncompressing: " + files[i]);
                    uncompressFile(files[i]);
                }
            }
		}
		catch(IOException e)
	...
	public static void uncompressFile(String filename) throws IOException
	{
		File fileOut = new File(uncompressedFilesDir,filename);
		InputStream in = assetManager.open(RESOURCES_DIR + "/" + filename, 	android.content.res.AssetManager.ACCESS_RANDOM);
